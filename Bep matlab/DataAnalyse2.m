...
%  vxw = zeros(length(v),1);
 ...
 for  g = 1:length(v)
     ...
     if u(g,1)<= 0.048*wf(g,1)                                 % At constant speed use acceleration kappa
     kappaf = 1- u(g,1)/(0.048*wf(g,1));
     kappa(g,1) = kappaf;
     
%      elseif u(g,1) > 0.048*wf(g,1)
%        kappaf = ((0.048*wf(g,1))/(u(g,1))) -1;
%        kappa(g,1) = kappaf;
 ...
 for  g2 = 1:length(v)
     ...
     if u(g2,1)<= 0.048*wr(g2,1)                                 % At constant speed use acceleration kappa
     kappar = 1- u(g2,1)/(0.048*wr(g2,1));
     kappa(g2,2) = kappar;
     
%      elseif u(g2,1) > 0.048*wr(g2,1)
%        kappar = ((0.048*wr(g2,1))/(u(g2,1))) -1;
%        kappa(g2,2) = kappar;
%     
